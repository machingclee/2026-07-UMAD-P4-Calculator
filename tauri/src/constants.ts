// ── Config ────────────────────────────────────────────────────────────────────
// Same knobs as ../main.py.
// Dev (`tauri dev` / `dev:web`): APP_WIDTH / APP_HEIGHT always apply.
// Release: 變更-resized size is stored in config.json and restored on launch.
...
export const OVERLAY_HINT = "金反，紫同";
...
export const ACTION_BTN_PAD_TOP = 5; // px; extra padding above the icon
...
export const ACTION_EXCLUDED_OPACITY = 0.4; // other round's ⏩ 💧 ⚡ when this round has one selected
export const ACTION_BADGE_TOP = -5; // px; 0 = top of icon, negative moves the label up
